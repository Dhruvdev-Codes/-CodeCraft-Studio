...
import { Trophy, Calendar, Clock, Award, ArrowRight, Sparkles } from "lucide-react";
...
export default async function ContestsPage() {
  ...
  try {
    ...
    console.error("[ContestsPage] Database load error:", err);
  }
  const now = new Date();
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-10">
        ...
        {contests.length === 0 ? (
          <Card className="p-12 text-center max-w-lg mx-auto">
            <Trophy className="h-12 w-12 mx-auto text-violet-500 mb-4 opacity-70" />
            <h3 className="text-lg font-bold">No Scheduled Contests Yet</h3>
            <p className="text-sm text-slate-500 mt-2">
              Contests will appear here once created. You can seed initial contest data or practice with standard problems.
            </p>
            <div className="mt-6 flex justify-center gap-3">
              <Link href="/problems">
                <Button className="gap-2">Practice Problems</Button>
              </Link>
              <Link href="/admin">
                <Button variant="outline" className="gap-2">
                  <Sparkles className="h-4 w-4" /> Seed Initial Contests
                </Button>
              </Link>
            </div>
          </Card>
