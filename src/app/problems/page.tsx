...
export default async function ProblemsPage() {
  ...
  try {
    await connectDb();
    problems = await Problem.find({}).select("-testCases").sort({ createdAt: -1 }).lean();
  } catch (err) {
    console.error("[ProblemsPage] Database load error:", err);
  }
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-10">
        ...
        {problems.length === 0 ? (
          <Card className="p-12 text-center max-w-lg mx-auto">
            <Terminal className="h-12 w-12 mx-auto text-brand-500 mb-4 opacity-70" />
            <h3 className="text-lg font-bold">No Problems Available Yet</h3>
            <p className="text-sm text-slate-500 mt-2">
              The problem database has not been seeded yet. You can seed initial algorithm problems from the admin console.
            </p>
            <div className="mt-6 flex justify-center gap-3">
              <Link href="/admin">
                <Button className="gap-2">
                  <Sparkles className="h-4 w-4" /> Seed Initial Problems
                </Button>
              </Link>
              <Link href="/ide">
